 ...
 *
 * The SessionEvent log keeps working in parallel: the harness's sync
 * /session/:id/message endpoint also feeds its `/event` SSE bus, which the
 * worker subscribes to and persists. The route here does NOT append events
 * itself — the harness is the single writer of `user_message` /
 * `assistant_text` / tool events into the log.
 *
